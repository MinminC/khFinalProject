...
<div id="wrap" style="margin-top:200px;">
	...
	<ul class="pagination">
		<c:if test="${pi.currentPage != 1}">
			<li class="page-item"><a class="page-link" href="list.no?pageNo=${pi.currentPage - 1}">&lt;</a></li>
		</c:if>
		<c:forEach var="i" begin="${pi.startPage}" end="${pi.endPage}">
			<li class="page-item"><a class="page-link" href="list.no?keyword=${keyword}&pageNo=${i}">${i}</a></li>
	...
	<c:if test="${not empty keyword}">
		<script>
			$('.pagination li').attr('href', 'search.no?keyword=${keyword}&pageNo=${i}');
			$('.pagination .firstPage').attr('href', 'search.no?keyword=${keyword}&pageNo=${pi.currentPage - 1}')
			$('.pagination .endPage').attr('href', 'search.no?keyword=${keyword}&pageNo=${pi.currentPage + 1}')
		</script>
	</c:if>
</div>
